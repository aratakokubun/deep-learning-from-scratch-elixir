'''
Note: Module copied from Numexy and extended partially.
'''
...
defmodule Np do
  ...
  '''
  Additional functions to align with Numpy of numpy
  '''
  def ndim(%Array{array: _, shape: {nil, nil}}), do: 0
  def ndim(%Array{array: _, shape: {_, nil}}), do: 1
  def ndim(%Array{array: _, shape: {_, _}}), do: 2

  def size(%Array{array: _, shape: {nil, nil}}), do: 0
  ...
  def arange(s, e, i) when s < e and i > 0 do
    s..e-1
    |> Enum.to_list
    |> Enum.filter(fn v -> rem(v-s, i) == 0 end)
    |> new
  end
  ...
  def reshape(mat = %Array{array: arr, shape: _}, target_row, target_col) do
    ...
    if Np.size(mat) == target_row*target_col do
      # TODO: Error handling
      {:error}
    end
    arr
  ...
  def mul(%Array{array: xm, shape: {xm_row, xm_col}}, %Array{array: ym, shape: {ym_row, nil}}) when xm_row == ym_row and xm_col != nil do
    xv = List.flatten(xm)
    yv = List.duplicate(ym, xm_col) |> List.flatten()
    Enum.zip(xv,yv)
  ...
  # Add matrix and vector with expansion
  def add(%Array{array: xm, shape: {xm_row, xm_col}}, %Array{array: ym, shape: {ym_row, nil}}) when xm_col == ym_row do
    xv = List.flatten(xm)
    yv = List.duplicate(ym, xm_row) |> List.flatten()
    Enum.zip(xv, yv)
    |> Enum.map(fn({a, b}) -> a + b end)
    |> Enum.chunk_every(xm_col)
    |> new
  end
  def add(%Array{array: xm, shape: {xm_row, nil}}, %Array{array: ym, shape: {ym_row, ym_col}}) when xm_row == ym_col do
    xv = List.duplicate(xm, ym_row) |> List.flatten()
    yv = List.flatten(ym)
    Enum.zip(xv, yv)
    |> Enum.map(fn({a, b}) -> a + b end)
    |> Enum.chunk_every(ym_col)
    |> new
  end
